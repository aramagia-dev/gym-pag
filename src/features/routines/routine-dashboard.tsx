...
export default function RoutineDashboard() {
  ...
  async function handleDelete(routine: RoutineTemplate) {
    if (!window.confirm(`¿Está seguro de que desea eliminar la rutina "${routine.name}"?`)) return;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section aria-labelledby="saved-routines-title">
          ...
          {loading && <p className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-sm text-slate-400">Cargando rutinas...</p>}
           ...
           {deleteError && <p role="alert" className="mb-4 rounded-lg border border-rose-900/70 bg-rose-950/50 px-3 py-2 text-sm text-rose-200">{deleteError}</p>}
