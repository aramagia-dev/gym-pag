...
export default function RoutineNavigation({ active }: { active: "home" | "routines" | "new" | "edit" }) {
  return (
    <nav className="flex flex-wrap gap-2 text-sm" aria-label="Navegación principal">
      ...
      <a className={`min-h-11 rounded-lg px-3 py-2.5 ${active === "routines" ? "bg-cyan-400 font-semibold text-slate-950" : "border border-slate-700 text-slate-300 hover:border-cyan-400"}`} href="/routines">Rutinas</a>
